...
const bookappointment = async (req, res) => {
  try {
    ...
    let randomNumber = Math.floor(Math.random() * (10000000 - 100 + 1)) + 100;
    let urlString = 'http://localhost:8080/?room=' + randomNumber;
    ...
    const userChatRoomNotification = Notification({
      userId: req.locals,
      content: `Join your Private ChatRoom with Dr. ${req.body.doctorname} at :- ${urlString}`,
    });

    await userChatRoomNotification.save();
    ...
    const doctorChatRoomNotification = Notification({
      userId: req.body.doctorId,
      content: `Join your Private ChatRoom with ${user.firstname} ${user.lastname} at :- ${urlString}`,
    });

    await doctorChatRoomNotification.save();
